...
save(genotype_mlp, genotype_data, general_data, file = "data/my_data.RData")
...
# Save the circular tree (Fig3a: branches only) and annotated tree (Fig3: with rings)
...
tab3 <- as.data.frame(coef(summary(glmm_model)))
tab3 <- cbind(Predictor = rownames(tab3), tab3)
write.csv(tab3, "output/tables/Table3_GLMM_binomial_regression.csv", row.names = FALSE)
...
# columns. write.table with sep="," preserves that structure in CSV format.
...
## This summarizes per_variant to understand better the diveristy come from the mutation or recombination
```{r}
# Is within-lineage diversity generated by mutation or by recombination?
# For each asexual lineage (MLL), we compare every distinct genotype to the
# lineage's most common ("central") genotype and count how many loci differ:
#   1 differing locus  -> single mutational step (mutation)
#   >=3 differing loci -> multilocus reshuffling (recombination-like)

library(adegenet); library(poppr); library(dplyr)

asexual_genind <- data_GenInd_Asex_MLL          # asexual isolates; pop() == organised_MLL
genotype_table <- genind2df(asexual_genind, sep = "/")
lineage_id     <- genotype_table$pop
genotype_table$pop <- NULL
locus_names    <- colnames(genotype_table)

# Make allele order irrelevant so 100/102 == 102/100
sort_alleles <- function(locus_column) {
  vapply(strsplit(locus_column, "/"),
         function(alleles) paste(sort(alleles), collapse = "/"),
         character(1))
}
genotype_sorted <- as.data.frame(lapply(genotype_table, sort_alleles),
                                 stringsAsFactors = FALSE)
colnames(genotype_sorted) <- locus_names

# For each lineage: compare every distinct genotype to the central (most common) one
variant_differences <- do.call(rbind, lapply(unique(lineage_id), function(this_lineage) {
  lineage_genotypes <- genotype_sorted[lineage_id == this_lineage, , drop = FALSE]
  if (nrow(lineage_genotypes) < 2) return(NULL)

  genotype_string   <- apply(lineage_genotypes, 1, paste, collapse = "|")
  genotype_counts   <- sort(table(genotype_string), decreasing = TRUE)
  if (length(genotype_counts) < 2) return(NULL)          # single genotype: no diversity

  central_alleles   <- strsplit(names(genotype_counts)[1], "\\|")[[1]]
  variant_genotypes <- names(genotype_counts)[-1]

  do.call(rbind, lapply(variant_genotypes, function(variant) {
    variant_alleles <- strsplit(variant, "\\|")[[1]]
    comparable      <- !(is.na(central_alleles) | is.na(variant_alleles) |
                         central_alleles == "NA" | variant_alleles == "NA")
    data.frame(lineage           = this_lineage,
               n_isolates_variant = as.integer(genotype_counts[variant]),
               loci_differing     = sum(central_alleles[comparable] != variant_alleles[comparable]))
  }))
}))

# Overall breakdown (these are the numbers quoted in the Discussion)
cat(sprintf("1 locus: %.1f%%   2 loci: %.1f%%   >=3 loci: %.1f%%   (n = %d variants)\n",
    100 * mean(variant_differences$loci_differing == 1,  na.rm = TRUE),
    100 * mean(variant_differences$loci_differing == 2,  na.rm = TRUE),
    100 * mean(variant_differences$loci_differing >= 3,  na.rm = TRUE),
    sum(!is.na(variant_differences$loci_differing))))

# Table S2: one row per asexual lineage
within_lineage_summary <- variant_differences %>%
  group_by(lineage) %>%
  summarise(
    n_variant_genotypes = n(),
    variants_1_locus    = sum(loci_differing == 1,  na.rm = TRUE),
    variants_2_loci     = sum(loci_differing == 2,  na.rm = TRUE),
    variants_3plus_loci = sum(loci_differing >= 3,  na.rm = TRUE),
    max_loci_differing  = ifelse(all(is.na(loci_differing)), 0L,
                                 max(loci_differing, na.rm = TRUE)),
    .groups = "drop"
  )

# Add total isolates per lineage, keep the real MLL (organised_MLL) labels
isolates_per_lineage <- as.data.frame(table(pop(asexual_genind)))
colnames(isolates_per_lineage) <- c("lineage", "n_isolates_total")
isolates_per_lineage$lineage <- as.integer(as.character(isolates_per_lineage$lineage))

tableS2 <- merge(isolates_per_lineage, within_lineage_summary, by = "lineage", all.x = TRUE)
tableS2[is.na(tableS2)] <- 0                              # lineages with no internal variants
tableS2$Lineage <- paste0("MLL", tableS2$lineage)        # real labels, not renumbered
tableS2 <- tableS2[order(-tableS2$n_isolates_total), ]
tableS2 <- tableS2[, c("Lineage", "n_isolates_total", "n_variant_genotypes",
                       "variants_1_locus", "variants_2_loci",
                       "variants_3plus_loci", "max_loci_differing")]

print(tableS2)
write.csv(tableS2, "output/tables/Table_S2_within_lineage_variation_asexual.csv", row.names = FALSE)
```
