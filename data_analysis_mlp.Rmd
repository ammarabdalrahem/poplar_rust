...
required_packages <- c("knitr","ggplot2","readxl","tidyverse","genepop","hierfstat","here","mapdata",
                       ...
                       "ade4","remotes","ggtree","ggtreeExtra","viridis","factoextra","openxlsx","ggrepel","RClone","ggsci","scales","lme4")    
...
#> length(unique(genotype_data[genotype_data$Cluster == "Cluster 1", ]$MLG))
#[1] 63
#> length(unique(genotype_data[genotype_data$Cluster == "Cluster 2", ]$MLG))
#[1] 1406
#> length(unique(genotype_data[genotype_data$Cluster == "Cluster 1", ]$organised_MLL))
#[1] 58
#> length(unique(genotype_data[genotype_data$Cluster == "Cluster 2", ]$organised_MLL))
#[1] 1400
...
npg_colors <- pal_npg("nrc")(9)  # Extract NPG colors, 8 colors for top 8 MLLs later


reproduction_colors <- c("Asex" = npg_colors[1], "Sex" = npg_colors[2])


p <- ggtree(tree, aes(color = group), layout = "circular") +
  scale_color_manual(values = reproduction_colors) +  # Apply the manual colors for 'Sex' and 'Asex'
  labs(color = "Reproduction") +  # Title for the color legend
  theme(
    ...
    legend.text = element_text(color = 'black', face = 'bold', size = 13),  # Legend text style
    legend.title = element_text(color = 'black', face = 'bold', size = 15)  # Legend title style
  )
...
specific_lineage_genind <- df2genind(
  ...
  pop = specific_lineage$MLG,                       # giving the population of each individual
  NA.char = "999",                      # string corresponding to missing allele 999 or 999999
  ploidy = 2,
  type = "codom",                       #codom' stands for 'codominant' (e.g. microstallites, allozymes)
  strata = NULL,
...
mst <- poppr.msn(specific_lineage_genind, dist_matrix,threshold=0,
                 clustering.algorithm="average_neighbor",
                 vertex.label=NA
                 )
