---
title: "Population Genetics Analysis of Mlp"
output: 
  html_document:
...
## Introduction
This document presents the population genetics analysis of **Mlp** using SSR marker data. The analysis includes genetic diversity, spatial structure, and visualization of results. 
------------------------------------------------------------------------
...
#Define the MLL threshold 5.5, “UPGMA ” algorithm as old data
...
##4- Identification the individuals through assignments into two clusters
...
repeated_MLL_table <- genotype_data [,c("MLL","Year")]
...
##corss validation to define Mll_persistence
...
for (i in 1:nrow(mll_year_df)) {
  ...
  genotype_data$Mll_persistence[genotype_data$MLL == mll_value] <- Mll_persistence_value
}
...
#Rows represent the assigned categories (Asex, Sex)
#Columns represent the mll_presistance categories (Asex, Sex)
#329 individuals are Asex by both assigned and mll_presistance
#110 individuals are Asex by assigned but Sex in mll_presistance
#12 individuals are Sex by assigned but Asex in mll_presistance
#1630 individuals are Sex by both assigned and mll_presistance
...
### Define reproduction mode based on assignment and Mll_persistence in third cloumn
...
# If the individual is assigned to the Asex cluster or has Asex MLL persistence, assign "Asex" to the Reproduction column
# because it could be asexually reproducing but didn't sampled so it is assigned as asex but not showed as asex in the MLL_persistence
# on the other hand if the individual is exist in the MLL_persistence but not assigned as asex then it is asexually reproducing biologically 

```


##G/N and Fis calculation for Reproduction groups
...
##G/N and Fis calculation for Mll for Sexual group (as Reproduction)
...
rm (Ar_per_loc, Asexual_lineages, data_Fstat, data_Genclone_Asex, data_Genclone_Sex,
...
p2 <- p2 +
  labs(fill = "Location", color = "Reproduction") 
...
head(MST_data)
# Think I need to keep only 2009 2011 data
...
dist <- diss.dist(data_GenInd_Asex_MLL)
tree <- nj(dist)
tree2 <- aboot(data_GenInd_Asex_MLL, dist = diss.dist , tree = "upgma", missing = "asis")

# Create group information based on 'Profil' column
groupInfo <- split(rownames(Asex_data), Asex_data$organised_MLL)

# Group the tree labels based on the group information
tree <- groupOTU(tree2, groupInfo)

# Step 1: Filter the organised_MLL column
label_info <- data.frame(
    label = tree$tip.label,
    organised_MLL = Asex_data[tree$tip.label, "organised_MLL"]
)
label_info$organised_MLL <- ifelse(
    label_info$organised_MLL %in% 1:7,
    label_info$organised_MLL,
    "Other"
)

# Step 2: Create group information
groupInfo <- split(label_info$label, label_info$organised_MLL)

# Step 3: Group the tree labels
tree <- groupOTU(tree, groupInfo)

# Step 4: Define a color palette
mll_colors <- c(
    "1" = "red",
    "2" = "blue",
    "3" = "green",
...
# Step 5: Create the ggtree plot
p <- ggtree(tree, aes(color = group), layout = "circular") +
    scale_color_manual(values = mll_colors) +
    labs(color = "organised_MLL") +
    theme(
