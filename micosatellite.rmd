...
#need to remove missing data 999999

```{r}
#create table of ClonEstiMate software by select columns with "Mlp" and Year
ClonEstiMate_data <- c("Year", grep("Mlp", names(data), value = TRUE))
ClonEstiMate_data <- data[, ClonEstiMate_data]

#Keep 2009, and 2011 only and remove the rest
ClonEstiMate_data <- ClonEstiMate_data[ClonEstiMate_data$Year %in% c("2009", "2011"), ]

#add pop col
ClonEstiMate_data <- ClonEstiMate_data %>% 
  mutate(pop = if_else(Year == 2009, "France", "France"))

#Change pop col order
ClonEstiMate_data <- ClonEstiMate_data %>% relocate(pop)



#separate each 6 digits of each loci into two cols
#1.Define the column names
mlp_columns <- names(ClonEstiMate_data)[grepl("^Mlp_", names(ClonEstiMate_data))]

#2.Loop over each MLP column 
for(col in mlp_columns) {
  # Create new column names
  new_cols <- paste0(col, "_part", 1:2)
  
  ClonEstiMate_data <- ClonEstiMate_data %>%
    mutate(
      !!new_cols[1] := as.integer(substr(!!sym(col), 1, 3)),
      !!new_cols[2] := as.integer(substr(!!sym(col), 4, 6))
    ) %>%
    select(-!!sym(col))  # Remove the original column
}

write.table(ClonEstiMate_data, "ClonEstiMate_data.txt", sep = "\t",quote = FALSE, row.names = FALSE, col.names =TRUE)
...
ASex_data$mll <- data$Mll[match(row.names(ASex_data), data$Isolate)]
# Create a contingency table
contingency_table_ASex <- table(ASex_data$MLL_poppr, ASex_data$mll)

# Print the contingency table
write.table(contingency_table_ASex, file = "contingency_table_ASex.csv", sep = ",", quote = FALSE)
...
```
#creat data for ClonEstiMate for asexual (clonal)

```{r}
ASex_data <- genotype_data[genotype_data$profile == "Asex", ]
#create table of ClonEstiMate software by select columns with "Mlp" and Year
ClonEstiMate_data_asexual <- c("year", grep("Mlp", names(ASex_data), value = TRUE))
ClonEstiMate_data_asexual <- ASex_data[, ClonEstiMate_data_asexual]

#Keep 2009, and 2011 only and remove the rest
ClonEstiMate_data_asexual <- ClonEstiMate_data_asexual[ClonEstiMate_data_asexual$year %in% c("2009", "2011"), ]

#add pop col
ClonEstiMate_data_asexual <- ClonEstiMate_data_asexual %>% 
  mutate(pop = if_else(year == 2009, "France", "France"))

#Change pop col order
ClonEstiMate_data_asexual <- ClonEstiMate_data_asexual %>% relocate(pop)



#separate each 6 digits of each loci into two cols
#1.Define the column names
mlp_columns <- names(ClonEstiMate_data_asexual)[grepl("^Mlp_", names(ClonEstiMate_data_asexual))]
...
write.table(ClonEstiMate_data_asexual, "ClonEstiMate_data_asexual.txt", sep = "\t",quote = FALSE, row.names = FALSE, col.names =TRUE)


```

#creat data for ClonEstiMate for sexual

```{r}
Sex_data <- genotype_data[genotype_data$profile == "Sex"& genotype_data$Reproduction== "Sex", ]
#create table of ClonEstiMate software by select columns with "Mlp" and Year
ClonEstiMate_data_sexual <- c("year", grep("Mlp", names(Sex_data), value = TRUE))
ClonEstiMate_data_sexual <- Sex_data[, ClonEstiMate_data_sexual]

#Keep 2009, and 2011 only and remove the rest
ClonEstiMate_data_sexual <- ClonEstiMate_data_sexual[ClonEstiMate_data_sexual$year %in% c("2009", "2011"), ]

#add pop col
ClonEstiMate_data_sexual <- ClonEstiMate_data_sexual %>% 
  mutate(pop = if_else(year == 2009, "France", "France"))

#Change pop col order
ClonEstiMate_data_sexual <- ClonEstiMate_data_sexual %>% relocate(pop)



#separate each 6 digits of each loci into two cols
#1.Define the column names
mlp_columns <- names(ClonEstiMate_data_sexual)[grepl("^Mlp_", names(ClonEstiMate_data_sexual))]
...
write.table(ClonEstiMate_data_sexual, "ClonEstiMate_data_sexual.txt", sep = "\t",quote = FALSE, row.names = FALSE, col.names =TRUE)
...
#Mental test (assess the correlation between genetic and geographic distance matrices)
